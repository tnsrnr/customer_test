...
const KpiCards: React.FC<KpiCardsProps> = ({ data, loading, error }) => {
  ...
  const calculateCurrentMonthKPIs = () => {
    ...
    const totalSalesRow = monthlyDetails.find(item => item.column0 === '합계' && item.column1 === '매출액');
    const totalProfitRow = monthlyDetails.find(item => item.column0 === '합계' && item.column1 === '영업이익');
    ...
    // 영업이익률 계산 (소수점 반올림)
    const currentOpMargin = currentSales > 0 ? Math.round((currentOpProfit / currentSales) * 100 * 10) / 10 : 0;
    const prevOpMargin = prevSales > 0 ? Math.round((prevOpProfit / prevSales) * 100 * 10) / 10 : 0;
    const opMarginChange = Math.round((currentOpMargin - prevOpMargin) * 10) / 10;
    
    // 법인 수 계산 (합계 제외, 고유한 구분 추출)
    const companyCount = Array.from(new Set(monthlyDetails.map(item => item.column0).filter(name => name !== '합계'))).length;
        ...
        <div className="relative flex items-center h-full">
          ...
          <div className="flex-1 flex items-center justify-center ml-4">
            <div className="text-center">
              <span className="text-sm font-medium text-emerald-200 mb-1 block">진출현황</span>
              <div className="flex items-center justify-center">
                <span className="text-3xl font-bold text-white drop-shadow-sm">{kpiMetrics.companyCount}</span>
                <span className="text-lg font-medium text-emerald-200 ml-1">개국</span>
              </div>
            </div>
          </div>
        ...
        <div className="relative flex items-center h-full">
          ...
          <div className="flex-1 flex items-center justify-center ml-4">
            <div className="text-center">
              ...
              <div className="flex items-center justify-center">
                <span className="text-3xl font-bold text-white drop-shadow-sm">
                  <CountUpAnimation end={kpiMetrics.currentOpMargin} decimals={1} />
                </span>
