...
// Domestic Subsidiaries Performance 데이터 타입 정의
export interface DomesticSubsidiariesData {
...
export const generateGridColumns = (monthLabels: string[]): GridColumn[] => {
  ...
  fixedColumns.forEach((columnKey, index) => {
    if (index === 0) {
      columns.push({ key: columnKey, label: '법인명' });
    } else if (index === 1) {
...
export const generateGridHeaders = (monthLabels: string[]): { topRow: GridColumn[] } => {
  return {
    topRow: [
      { key: 'company', label: '법인명', colSpan: 1 },
