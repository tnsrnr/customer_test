...
export interface MonthlyDetailData {
  column0: string;           // 고정 컬럼명 (COLUMN0) - 법인명
...
// Overseas Subsidiaries Performance 데이터 타입 정의
export interface OverseasSubsidiariesData {
...
export const generateGridColumns = (monthLabels: string[]): GridColumn[] => {
  ...
  // 고정 컬럼명에 동적 라벨 매핑 (COLUMN0~COLUMN8)
  ...
  fixedColumns.forEach((columnKey, index) => {
    if (index === 0) {
      ...
      columns.push({ key: columnKey, label: '계정명' });
    } else if (index <= 6) {
      columns.push({ 
        key: columnKey, 
      ...
      columns.push({ key: columnKey, label: '전월대비' });
    }
  });
...
export const generateGridHeaders = (monthLabels: string[]): { topRow: GridColumn[] } => {
  return {
    topRow: [
      { key: 'company', label: '구분', colSpan: 1 },
      { key: 'category', label: '계정명', colSpan: 1 },
      { key: 'monthly', label: '월별', colSpan: monthLabels.length },
      { key: 'total', label: '합계', colSpan: 1 },
      { key: 'growth', label: '전월대비', colSpan: 1 }
    ]
  };
};
