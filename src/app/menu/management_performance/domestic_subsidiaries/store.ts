...
import { DomesticSubsidiariesData, ChartData } from './types';
...
const domestic_subsidiaries = async (year: number, month: number): Promise<DomesticSubsidiariesData> => {
  try {
    const params = createParams(year, month);
    const response = await fetch(`/auth/api/proxy?path=/api/MIS030231SVC/domestic_subsidiaries`, {
      method: 'POST',
...
interface DomesticSubsidiariesStore {
  ...
  setData: (data: DomesticSubsidiariesData) => void;
...
export const useDomesticSubsidiariesStore = create<DomesticSubsidiariesStore>((set, get) => {
  ...
  return {
    ...
    fetchAllData: async () => {
      ...
      try {
        ...
        const combinedData = await domestic_subsidiaries(currentYear, currentMonth);
