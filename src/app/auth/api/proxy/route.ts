...
async function handleProxyRequest(req: NextRequest, method: string) {
  try {
    ...
    const serverType = url.searchParams.get('server') || 'spring'; // 기본값은 spring
    ...
    if (serverType === 'spring') {
      // Spring 서버 요청 처리
      targetUrl = `https://qa-lv1.htns.com${targetPath}`;
      ...
      console.log(`🌐 일반 API 서버 프록시 요청: ${method} ${targetUrl}`);
    ...
    if (serverType === 'spring' && response.headers.get('set-cookie')) {
